window.onload = function() { //when page loads
    const date = new Date();
    //output our date in M/D/Y
    const dateString = (date.getMonth() + 1) + '/' + date.getDate() + '/' + date.getFullYear();
    document.getElementsByClassName("date")[0].innerHTML = dateString;
    
}


// addEventListener(submit, button, func)
function func(){

data = document.getElementById('button')
curCountry = data.value
console.log(curCountry)
}

let imageList = [];

for (let i = 0; i <= 7; i++) {
  var image = new Image(70, 70);
  switch (i) {
    case 1:
      image.src = "data:image/png;base64,";
      break;
    case 2:
      image.src = "https://upload.wikimedia.org/wikipedia/en/thumb/9/9e/Flag_of_Japan.svg/1200px-Flag_of_Japan.svg.png";
      break;
    case 3:
      image.src = "https://libertyflagandbanner.com/wp-content/uploads/2015/08/europe-flag.jpg";
      break;
    case 4:
      image.src = "https://cdn.britannica.com/90/7490-004-BAD4AA72/Flag-China.jpg";
      break;
    case 5:
      image.src = "data:image/png;base64,";
      break;
    case 6:
      image.src = "data:image/png;base64,";
      break;
    case 7:
      image.src = "https://upload.wikimedia.org/wikipedia/commons/thumb/a/a5/Flag_of_the_United_Kingdom_%281-2%29.svg/1200px-Flag_of_the_United_Kingdom_%281-2%29.svg.png";
      break;
    default:
      break;
  }
  imageList.push(image);
}

function switchImage() {
    var selectedImage = document.myForm.Currency1.options[document.myForm.Currency1.selectedIndex].value;
    document.getElementById('myImage').src = imageList[selectedImage].src;
    console.log(document.myImage.src)
}

//flag = document.getElementById('before')

function switchImage2() {
    console.log(document.myForm2)
    var selectedImage = document.myForm2.Currency2.options[document.myForm2.Currency2.selectedIndex].value;
    document.getElementById('myImage2').src = imageList[selectedImage].src;
    console.log(document.myImage.src)
}


